...
const generateReceiptBuffer = async ({ buyerName, buyerEmail, itemTitle, itemType, amount, sellerName, transactionId, date }) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Header background
      doc.rect(0, 0, 612, 120).fill('#63e5ff');
      
      // Header text
      doc.fontSize(28).fillColor('#0a4a57').text('SLIIT Learning Platform', 50, 35, { align: 'center' });
      doc.fontSize(14).fillColor('#0a4a57').text('Payment Receipt', 50, 75, { align: 'center' });
      ...
      doc.fontSize(12).fillColor('#0ab5d6').text('BUYER INFORMATION', 50, 200);
      ...
      doc.fontSize(12).fillColor('#0ab5d6').text('ITEM DETAILS', 50, 290);
      doc.fontSize(10).fillColor('#333333');
      ...
      doc.fontSize(12).fillColor('#0ab5d6').text('PAYMENT DETAILS', 50, 400);
      doc.fontSize(10).fillColor('#333333');
      doc.text('Payment Method: Bank Transfer', 50, 425);
      doc.text('Status: Verified ✓', 50, 445);
      
      doc.rect(50, 475, 512, 50).fill('#f3f4f6');
      doc.fontSize(16).fillColor('#0ab5d6').text(`Total Amount: LKR ${parseFloat(amount).toFixed(2)}`, 50, 490, { align: 'center' });
