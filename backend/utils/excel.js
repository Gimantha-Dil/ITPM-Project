...
const generateSalesReportBuffer = async (notes, sessions, sellerName) => {
  ...
  const notesSheet = workbook.addWorksheet('Notes Sales', {
    properties: { tabColor: { argb: '63E5FF' } }
  });
  ...
  // Header row — cyan bg, dark text
  notesSheet.getRow(1).font      = { bold: true, color: { argb: '0A4A57' } };
  notesSheet.getRow(1).fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: '63E5FF' } };
  ...
  noteTotal.font = { bold: true, color: { argb: '0AB5D6' } };
 
  // ---- Sessions Sheet ----
  const sessionsSheet = workbook.addWorksheet('Session Sales', {
    properties: { tabColor: { argb: 'B1F2FF' } }
  });
  ...
  // Header row — light cyan bg, dark text
  sessionsSheet.getRow(1).font      = { bold: true, color: { argb: '0A4A57' } };
  sessionsSheet.getRow(1).fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'B1F2FF' } };
  ...
  sessionTotal.font = { bold: true, color: { argb: '0AB5D6' } };
 
  // ---- Summary Sheet ----
  const summarySheet = workbook.addWorksheet('Summary', {
    properties: { tabColor: { argb: '0AB5D6' } }
  });
  ...
  summarySheet.getRow(1).font = { bold: true, color: { argb: '0A4A57' } };
  summarySheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '0AB5D6' } };
